...
WHERE property_name = 'WidgetSupport'
    AND NOT EXISTS(
        ...
        WHERE property_name = 'WidgetSupport' AND permission_name = 'CreateWindow'
    );
...
WHERE property_name = 'WidgetSupport'
    AND NOT EXISTS(
        SELECT 1
        FROM function_group_hmi_levels
        WHERE property_name = 'WidgetSupport' AND permission_name = 'DeleteWindow'
    );
...
WHERE property_name = 'WidgetSupport'
    AND NOT EXISTS(
        SELECT 1
        FROM function_group_hmi_levels
        WHERE property_name = 'WidgetSupport' AND permission_name = 'Show'
    );
...
WHERE property_name = 'WidgetSupport'
    AND NOT EXISTS(
        SELECT 1
        FROM function_group_hmi_levels
        WHERE property_name = 'WidgetSupport' AND permission_name = 'OnSystemCapabilityUpdated'
    );

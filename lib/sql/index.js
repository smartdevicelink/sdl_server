...
/*  CODE REVIEW:
    - we should clean up formatting of these queries for easier readability
    - remove doubleQuotes() function. use SQLbricks insert objects instead
    - use callbacks instead of returns for all SQL functions for future-proof flow control
    - what are setupSql* functions used for?
    - use bulk SQL insert statements instead of generating arrays of individual statements, where possible
    - avoid the use of Array().map() because it is synchronous
    - break groups of SQL into separate module files (e.g. applications, groups, messages, etc)
    - SQL in this file is v1-specific. Recommend to store it within the app/v1 directory tree
*/
...
module.exports = {
    ...
    setupSqlCommands: setupSqlCommands,
...
function setupSqlCommands (sqlStringArray) {
...
function insertFuncGroupInfo (objs) {
    return objs.map(function (obj) {
        ...
        .returning('*').toString(); //return the results of the inserts
    });
}
...
function getFuncGroupId (id) {
    return sql.select('*').from('function_group_info')
        .where({id: id})
        .orderBy('LOWER(function_group_info.property_name)').toString();
}
...
function getFuncGroupStatus (isProduction) {
    const funcGroupsProduction = sql.select('*').from('view_function_group_info')
        .where({status: 'PRODUCTION', 'is_deleted': false})
    ...
    const funcGroupsStaging = sql.select('view_function_group_info.*')
        ...
        .where(sql.or({'is_deleted': false, 'status': 'STAGING'}))
        .orderBy('LOWER(view_function_group_info.property_name)')
        .toString();
...
function getMessageGroupsById (id) {
    return sql.select('*').from('message_group')
        .where({id: id})
        .orderBy('LOWER(message_group.message_category)').toString();
}
...
function getFuncGroupHmiLevelsByIdsStagingFilter (ids) {
    return sql.select('function_group_hmi_levels.*').from('function_group_hmi_levels')
        .innerJoin('function_group_info', {'function_group_info.id': 'function_group_hmi_levels.function_group_id'})
        .where(
            sql.and(
                sql.in('id', ids),
                {status: 'STAGING'}
...
function getFuncGroupParametersByIdsStagingFilter (ids) {
    return sql.select('function_group_parameters.*').from('function_group_parameters')
        .innerJoin('function_group_info', {'function_group_info.id': 'function_group_parameters.function_group_id'})
        .where(
            sql.and(
                sql.in('id', ids),
                {status: 'STAGING'}

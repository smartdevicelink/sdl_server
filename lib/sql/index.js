...
module.exports = {
    ...
    getMessages: {
        ...
        categoryChangesOnly: getMessageCategoryChangesOnly
    },
...
function getMessagesStatus (isProduction) {
    const messagesProduction = sql.select().from('view_message_text')
        .where({status: 'PRODUCTION'})
        .toString();

    const messagesGroup = sql.select('max(id) AS id', 'message_category', 'language_id')
        .from('view_message_text')
        .groupBy('message_category', 'language_id');
    const messagesStaging = sql.select()
        .from('(' + messagesGroup + ') vmt')
        .innerJoin('view_message_text', {'view_message_text.id': 'vmt.id'})
        .toString();    

    if (isProduction) {
        //filter out so only production entries exist
        return messagesProduction;
    }
    else {
        //filter out based on highest id
        return messagesStaging;
    }  
}

function getMessageCategories (isProduction) {
    //retrieve only categories
    const messagesProduction = sql.select().from('view_message_text').distinct('message_category')
        .where({status: 'PRODUCTION'})
        .toString();

    const messagesGroup = sql.select('max(id) AS id', 'message_category')
        .from('view_message_text')
        .groupBy('message_category');
    const messagesStaging = sql.select()
        .from('(' + messagesGroup + ') vmt').distinct('message_category')
        .toString();
...
function getMessageByCategory (isProduction, category, preventStringify) {
    //retrieve all messages by a category
    let messagesProduction = sql.select('*').from('view_message_text')
        .where({status: 'PRODUCTION', message_category: category});

    const categoryFilter = sql.select('*').from('view_message_text')
        .where({message_category: category});
    const languageGroup = sql.select('max(id) AS id', 'language_id')
        .from('(' + categoryFilter + ') vmt')
        .groupBy('language_id');
    let messagesStaging = sql.select('view_message_text.*')
        .from('(' + languageGroup + ') vmt2')
        .innerJoin('view_message_text', {'view_message_text.id': 'vmt2.id'});

    if (!preventStringify) {
        messagesProduction = messagesProduction.toString();
        messagesStaging = messagesStaging.toString();
    }
...
function getMessageCategoriesByLanguage (isProduction, languageCode) {
    const messagesProduction = sql.select('*').from('view_message_text')
        .where({status: 'PRODUCTION', language_id: languageCode})
        .toString();

    const languageFilter = sql.select('*').from('view_message_text')
        .where({language_id: languageCode});
    const messagesGroup = sql.select('max(id) AS id', 'message_category')
        .from('(' + languageFilter + ') vmt')
        .groupBy('message_category');
    const messagesStaging = sql.select('view_message_text.*')
        .from('(' + messagesGroup + ') vmt2')
        .innerJoin('view_message_text', {'view_message_text.id': 'vmt2.id'})
        .toString();
...
function getMessageCategoriesByMaxId (isProduction) {
    ...
    if (isProduction) {
        messagesGroup = messagesGroup.where({status: 'PRODUCTION'})
    }
    messagesGroup = messagesGroup.groupBy('message_category');

    return sql.select('view_message_text.*')
        .from('(' + messagesGroup + ') vmt')
        .innerJoin('view_message_text', {'view_message_text.id': 'vmt.id'})
        .toString();
}

function getMessageCategoryChangesOnly () {
    //get either staging status entries (changed) or entries with is_deleted to true (changed)
    return sql.select().from('view_message_text')
        .where(sql.or({status: 'STAGING', is_deleted: true}))
        .toString();
}

function insertMessages (messages) {
    return messages.map(function (msg) {
        return sql.insert('message_text', msg).toString();
    });
}
